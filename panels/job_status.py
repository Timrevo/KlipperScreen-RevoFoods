...
class Panel(ScreenPanel):
    ...
    # Add extruder controls to grid with increased spacing between inputs
        ...
        self.save_btn_box = Gtk.Box(halign=Gtk.Align.CENTER)
        ...
        self.save_btn_box.pack_start(self.save_rates_button, False, False, 0)
        extruder_container.add(self.save_btn_box)
    ...
    def on_orange_rate_changed(self, value):
        ...
        if self.is_printing():
            if self.is_orange_active():
                ...
                self._screen._ws.klippy.gcode_script(f"SET_GCODE_VARIABLE MACRO=_exvar VARIABLE=curr_ospeed VALUE={new_rate}")
                logging.info(f"ORANGE: Updated variable to {new_rate}% (white is printing, no apply)")
        else:
            # Not printing: Safe to use full macro system
            try:
                self._screen._ws.klippy.gcode_script(f"SET_GCODE_VARIABLE MACRO=_exvar VARIABLE=curr_ospeed VALUE={new_rate}")
                ...
                logging.info(f"ORANGE: Set to {new_rate}% via T0 macro (not printing)")
            except Exception as e:
    ...
    def on_white_rate_changed(self, value):
        new_rate = int(value)
        if self.is_printing():
            if self.is_white_active():
                self._screen._ws.klippy.gcode_script(f"SET_GCODE_VARIABLE MACRO=_exvar VARIABLE=curr_wspeed VALUE={new_rate}")
                self._screen._ws.klippy.gcode_script(f"M221 S{new_rate}")
            else:
                # White is NOT active - only update variables, don't apply to printer
                self._screen._ws.klippy.gcode_script(f"SET_GCODE_VARIABLE MACRO=_exvar VARIABLE=curr_wspeed VALUE={new_rate}")
                logging.info(f" WHITE: Updated variable to {new_rate}% (orange is printing, no apply)")
        else:
            # Not printing: Safe to use full macro system
            try:
                self._screen._ws.klippy.gcode_script(f"SET_GCODE_VARIABLE MACRO=_exvar VARIABLE=curr_wspeed VALUE={new_rate}")
    ...
    def is_orange_active(self):
        """Simple check: Is orange extruder currently active?"""
        return self.current_extruder == "extruder"

    def is_white_active(self):
        """Simple check: Is white extruder currently active?"""
        return self.current_extruder == "extruder1"

    def is_printing(self):
        """Simple check: Is printer currently printing?"""
    ...
    def show_buttons_for_state(self):
        ...
        if self.state == "complete":
            ...
            self.save_btn_box.set_visible(False)
            ...
            if self.state == "printing":
                ...
                self.save_btn_box.set_visible(True)
        ...
        # Apply visibility settings AFTER show_all() to ensure they take effect
        if self.state == "complete":
            self.extruder_box.set_visible(False)
            self.save_rates_button.set_visible(False)
            self.save_btn_box.set_visible(False)
